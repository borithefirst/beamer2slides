...
%<!recast>\begin{frame}{\T{Results}}
%<recast>\begin{frame}{\T{The third table of numbers}}
%<!recast>  This third slide called Results has no label at all, so only its content can identify it.
%<recast>  This third slide called Results has no label at all, and the source has now given it another
%<recast>  title and rewritten the rest of what it says about itself.
  \note{Three slides are called Results. That is the test.}
\end{frame}
...
% `duplabel` takes the label without the frame before it giving it up: one label, two frames.
%<*!duplabel>
%<!movelabel>\begin{frame}[label=arriving]{\T{Arriving labels}}
%<movelabel>\begin{frame}[label=mobile]{\T{Arriving labels}}
%</!duplabel>
%<duplabel>\begin{frame}[label=mobile]{\T{Arriving labels}}
