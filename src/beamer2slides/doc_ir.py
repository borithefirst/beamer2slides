...
def _ordered(lists: dict, list_id: str | None, level: int) -> bool:
    """Ordered levels carry glyphType, unordered carry glyphSymbol."""
...
def _list_html(items: list[dict], depth: int) -> list[str]:
    ...
    while index < len(items):
        ...
        while deeper < len(items) and items[deeper].get("level", 0) > level:
            deeper += 1
        lines.append(pad + f" <li>{_runs_html(items[index]['runs'])}</li>")
        if deeper > index + 1:
