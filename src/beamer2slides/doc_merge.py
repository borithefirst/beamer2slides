...
def inherit_keys(base: dict, ours: dict) -> dict:
    ...
    for block in free:
        by_text.setdefault((_shape(block), block_text(block)), []).append(block)
    pending = []
    for block in ours["blocks"]:
        same = by_text.get((_shape(block), block_text(block)))
        if same:
    ...
    for block in pending:
        ...
        for candidate in rest:
            if _shape(candidate) != _shape(block):
                continue
...
def text_requests(live: dict, target: str) -> list[dict]:
    ...
    for op, i1, i2, j1, j2 in reversed(SequenceMatcher(None, a, b, autojunk=False).get_opcodes()):
        ...
        if insert:
            out.append({"insertText": {"location": {"index": start}, "text": insert}})
    return out
